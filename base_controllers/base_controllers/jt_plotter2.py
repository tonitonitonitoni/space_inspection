...
class JointTrajectoryPublisher(Node):
    def __init__(self):
        ...
        # Define the trajectory points to go through (target positions)
        ...
        self.target_positions = {joint: [] for joint in self.trajectory.joint_names}
        self.target_time_data = []  # Store times when the target positions are reached
    ...
    def joint_state_callback(self, msg: JointState):
        ...
        # Append target positions (we use the current trajectory points)
        target_point = self.trajectory_points[self.current_point % len(self.trajectory_points)]
        for i, joint in enumerate(self.trajectory.joint_names):
            self.target_positions[joint].append(target_point[i])
    ...
    def update_plot(self):
        ...
        for i, joint in enumerate(self.trajectory.joint_names):
            ...
            # Plot the current joint positions
            self.ax[i].plot(self.time_data, self.joint_positions[joint], label=f'{joint} Position', color='blue')
            # Plot the target joint positions in red
            self.ax[i].scatter(self.target_time_data, self.target_positions[joint], label=f'{joint} Target', color='red', marker='x')
            self.ax[i].set_title(f'{joint} Position')
            self.ax[i].set_xlabel('Time (s)')
            self.ax[i].set_ylabel('Position')
            self.ax[i].grid(True)
    ...
    def publish_trajectory(self):
        if self.current_point < len(self.trajectory_points):
            ...
            # Track the time for target position plotting
            self.target_time_data.append(self.get_clock().now().seconds_nanoseconds()[0])
        else:
