...
class PositionControlNode(Node):
    def __init__(self):
       ...
       # PID control constants for linear and angular velocities
        self.pid_linear = PIDController(Kp=1.0, Ki=0.0, Kd=0.1)
        self.pid_angular = PIDController(Kp=1.0, Ki=0.0, Kd=0.1)
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if distance_to_target > 0.1 or abs(angle_diff) > 0.1:  # If not at target yet
           ...
           # Use odometry information to compute control velocity
            ...
            # Control linear x direction (forward/backward)
            actuator_values[4] = self.pid_linear.should_activate(linear_velocity_x_cmd)
            actuator_values[5] = self.pid_linear.should_activate(linear_velocity_x_cmd)
            actuator_values[6] = self.pid_linear.should_activate(-linear_velocity_x_cmd)
            actuator_values[7] = self.pid_linear.should_activate(-linear_velocity_x_cmd)

            # Control linear y direction (side to side)
            actuator_values[2] = self.pid_linear.should_activate(linear_velocity_y_cmd)
            actuator_values[3] = self.pid_linear.should_activate(linear_velocity_y_cmd)
            actuator_values[0] = self.pid_linear.should_activate(-linear_velocity_y_cmd)
            actuator_values[1] = self.pid_linear.should_activate(-linear_velocity_y_cmd)

            # Control angular z direction (rotation)
            actuator_values[1] = max(actuator_values[1], self.pid_angular.should_activate(angular_velocity_cmd))
            actuator_values[2] = max(actuator_values[2], self.pid_angular.should_activate(angular_velocity_cmd))
            actuator_values[5] = max(actuator_values[5], self.pid_angular.should_activate(angular_velocity_cmd))
            actuator_values[6] = max(actuator_values[6], self.pid_angular.should_activate(angular_velocity_cmd))

            actuator_values[0] = max(actuator_values[0], self.pid_angular.should_activate(-angular_velocity_cmd))
            actuator_values[3] = max(actuator_values[3], self.pid_angular.should_activate(-angular_velocity_cmd))
            actuator_values[4] = max(actuator_values[4], self.pid_angular.should_activate(-angular_velocity_cmd))
            actuator_values[7] = max(actuator_values[7], self.pid_angular.should_activate(-angular_velocity_cmd))
